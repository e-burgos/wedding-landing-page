...
const useStyles = makeStyles((theme) => ({
  ...
  bgParty: {
    ...
    indexZ: -2,
    animation: 'slide-in-right 2s cubic-bezier(0.250, 0.460, 0.450, 0.940) 1s both',
  },
  bgShadow: {
    position: 'absolute',
    height: '100%',
      ...
      <Grid className={classes.background}>
        <div className={classes.cardFlip}>
          <ReactCardFlip
            ...
            <div onPointerOut={(e) => setFlip('')} onFocus={() => void 0} aria-hidden='true' className={classes.card}>
              <img src={partySection.icons.card} alt='card' className={classes.cardImg} />
            </div>
          </ReactCardFlip>
        </div>
        <div ref={refSection} className={`${isSectionVisible ? `${classes.bgShadow} ` : ''}`} />
        <div ref={refSection} className={`${isSectionVisible ? ` ${classes.bgParty}` : ''}`} />
      </Grid>
    </div>
  );
};
